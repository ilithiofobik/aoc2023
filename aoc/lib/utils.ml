let file_to_lines filename =
  ...
  let rec read_lines lines =
    ...
    | End_of_file ->
      close_in channel;
      lines |> List.rev |> List.map String.trim in
  read_lines []

let file_to_string filename = filename |> file_to_lines |> String.concat ""

let file_to_chars filename =
  file_to_string filename |> String.to_seq |> List.of_seq
